...
pytest.importorskip('tqdm', reason = 'tqdm is required for CLI batch runner tests')

from facefusion.download import conditional_download
from facefusion.jobs.job_manager import clear_jobs, init_jobs
from .helper import get_test_example_file, get_test_examples_directory, get_test_jobs_directory, get_test_output_file, get_test_outputs_directory, is_test_output_file, prepare_test_output_directory
...
def test_batch_run_directory_output() -> None:
        output_directory = os.path.join(get_test_outputs_directory(), 'batch-run-directory')
        ...
        commands = [
                ...
                'face_debugger',
                '-t',
                get_test_example_file('target-240p-batch-*.jpg'),
                '-o',
                output_directory
        ]

        assert subprocess.run(commands).returncode == 0
        assert is_test_output_file('batch-run-directory/target-240p-batch-1-faceswap-000.jpg') is True
        assert is_test_output_file('batch-run-directory/target-240p-batch-2-faceswap-001.jpg') is True
