...
def run_step(job_id: str, step_index: int, step: JobStep, process_step: ProcessStep) -> bool:
    ...
    logger.debug(f'[BATCH DEBUG] Resetting process_manager for step {step_index}', __name__)
    process_manager.end()
    ...
    logger.debug(f'[BATCH DEBUG] Ending process_manager for step {step_index}', __name__)
    process_manager.end()
