...
def finalize_image(target_path : str, output_path : str, output_image_resolution : Resolution) -> bool:
    logger.info('[BATCH DEBUG] finalize_image called:', __name__)
    logger.info(f'[BATCH DEBUG]   target_path: {target_path}', __name__)
    logger.info(f'[BATCH DEBUG]   output_path: {output_path}', __name__)
    logger.info(f'[BATCH DEBUG]   resolution: {output_image_resolution}', __name__)
    ...
    result = run_ffmpeg(commands).returncode == 0

    if os.path.exists(output_path):
        logger.info(f'[BATCH DEBUG] ✅ finalize_image SUCCESS - file exists: {output_path}', __name__)
    else:
        logger.error(f'[BATCH DEBUG] ❌ finalize_image FAILED - file missing: {output_path}', __name__)
