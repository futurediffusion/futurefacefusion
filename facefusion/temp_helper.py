...
def get_temp_directory_path(file_path : str) -> str:
    ...
    temp_path = os.path.join(state_manager.get_item('temp_path'), 'facefusion', temp_file_name)
    logger.debug(f'[BATCH DEBUG] Temp directory for {file_path}: {temp_path}', __name__)
    return temp_path
